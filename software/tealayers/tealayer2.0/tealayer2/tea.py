# Contains the Code for Tea Layer for use in TeaLarning and modern TensorFlow / Keras (TF 2.16+)
#
# Ported from tealayer2.0 (originally written for tensorflow-gpu==2.0.0b1).
# The math and constraints are UNCHANGED from the original tealayer2.0 implementation.
# Only two things changed, because their old APIs were removed from modern TF/Keras:
#   1) tf.keras.backend.in_train_phase(...)  -> removed. Replaced with the standard
#      Keras `training` argument passed into call().
#   2) tf.compat.v1.get_default_graph().gradient_override_map(...) + @tf.RegisterGradient
#      -> graph-mode-only trick, fragile under eager execution. Replaced with the
#      standard eager-friendly @tf.custom_gradient decorator, applied directly to a
#      custom `_ste_round` op that implements the exact same Straight-Through Estimator
#      (forward = round(), backward = gradient passed through unchanged).
...
#   Forward:  behaves exactly like tf.round(x)
#   Backward: gradient is passed straight through unchanged (as if round() were identity),
#             which is exactly what the original CustomRound gradient override did.
@tf.custom_gradient
def _ste_round(x):
    def grad(upstream, variables=None):
        # `variables` must be accepted because this function is sometimes called
        # directly on a tf.Variable (e.g. self.connections). We don't need to
        # return gradients for it separately: passing `upstream` straight through
        # for `x` is exactly the Straight-Through Estimator, and returning an
        # empty list for `variables` tells TF "no extra gradient contribution here".
...
class Tea(layers.Layer):
    ...
    def call(self, inputs, training=None):
        ...
        # Constrain the Input:
        if self.round_input:
            inputs = _ste_round(inputs)
        else:
            inputs = tf.cond(tf.cast(training, tf.bool),
                             lambda: inputs,
                             lambda: _ste_round(inputs))

        # Connection Constraints:
        connections = self.connections

        if self.round_connections:
            connections = _ste_round(connections)
        else:
            connections = tf.cond(tf.cast(training, tf.bool),
                                  lambda: connections,
                                  lambda: _ste_round(connections))
        ...
        if self.clip_connections:
            ...
            connections = tf.cond(tf.cast(training, tf.bool),
                                  lambda: connections,
                                  lambda: tf.keras.backend.clip(connections, 0, 1))
        ...
        if self.use_bias:
            ...
            if self.round_bias:
                biases = _ste_round(self.biases)
            else:
                biases = tf.cond(tf.cast(training, tf.bool),
                                 lambda: self.biases,
                                 lambda: _ste_round(self.biases))

            output = tf.keras.backend.bias_add(output, biases, data_format='channels_last')

        # Apply activation / Spike(s):
        # Training: sigmoid estimate (differentiable). Inference: hard binary spike (>= 0).
        output = tf.cond(tf.cast(training, tf.bool),
                         lambda: self.activation(output),
                         lambda: tf.dtypes.cast(tf.math.greater_equal(output, 0.0), tf.float32))
    ...
    ##################################################################################################################################
    def get_config(self):
        config = super(Tea, self).get_config()
        config.update({
            'units': self.units,
            'use_bias': self.use_bias,
            'round_input': self.round_input,
            'round_connections': self.round_connections,
            'clip_connections': self.clip_connections,
            'round_bias': self.round_bias,
            'constrain_after_train': self.constrain_after_train,
        })
        return config
    ##################################################################################################################################
# END CLASS : TEA
