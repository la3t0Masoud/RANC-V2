...
def get_connections_and_biases(model, num_layers):
    ...
    # New Keras versions no longer prefix weight.name with the layer name
    # (e.g. it's just "connections" instead of "tea_1_1/connections:0"),
    # so we pair each weight with its owning layer's name directly.
    names = [layer.name for layer in model.layers for weight in layer.weights]
    data_types = [weight.name for layer in model.layers for weight in layer.weights]
    connections = [[] for i in range(num_layers)]
    biases = [[] for i in range(num_layers)]

    for weight, name, data_type in zip(weights, names, data_types):
        layer_id = int(name.split('_')[1]) - 1
